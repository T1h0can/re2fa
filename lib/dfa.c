...
static int
dfa_add_trans_native(struct dfa *dfa, size_t state_size, int bps, size_t from, unsigned char mark, size_t to);
...
int dfa_add_state(struct dfa *dfa, uint64_t *index) {
...
int dfa_add_n_state(struct dfa *dfa, size_t cnt, uint64_t *index) {
    if (index != NULL)
        *index = dfa->state_cnt;
